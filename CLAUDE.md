...
- Rendered mp3s (`audiobook/narration/output/Chapter-NN-*.mp3`) **are committed to the repo**, alongside `tagged/*.txt` — also hand new ones to the user as files directly when you finish a batch. Don't commit the per-chunk working files (`output/chNN/`, concat lists, silence pads) — just the final chapter mp3s.
